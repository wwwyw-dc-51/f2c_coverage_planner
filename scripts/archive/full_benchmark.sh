...
echo "========================================="
echo "  F2C Full Benchmark: 7 scenarios x 2 modes"
echo "  Output: $RESULT_DIR"
echo "========================================="

SUMMARY_JSON="$RESULT_DIR/summary.json"
echo "{" > "$SUMMARY_JSON"
echo  timestamp: '2026-07-13T17:31:43+08:00', >> "$SUMMARY_JSON"
echo  params: robot_width:0.95, >> "$SUMMARY_JSON"
echo  coverage_width:0.45, >> "$SUMMARY_JSON"
echo  sweep:true, >> "$SUMMARY_JSON"
echo  RDP:0.05, >> "$SUMMARY_JSON"
echo  turn:direct, >> "$SUMMARY_JSON"
...
for NAME in notched S1 S2 S3 S4 S5 S6; do
    YAML="${YAMLS[$NAME]}"
    BOUNDARY=$(python3 -c "import yaml; print(yaml.safe_load(open())[boundary_type])" 2>/dev/null || echo "closed")
...
PYEOF
        ...
        # v2 fix: poll for result then kill planner
        for i in $(seq 1 45); do
            if grep -q "综合得分" "$LOG" 2>/dev/null; then sleep 1; break; fi
            sleep 2
        done
