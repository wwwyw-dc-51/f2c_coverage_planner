...
> **版本**: v9.11++ 边界补线优化版 (2026-07-20)
> **编译状态**: 0 error 0 warning | **基准**: 8 场景零回归，hole_crossings=0
> **当前稳定分支**: `main-stable` @ 703b44d
...
| 问题 | 严重度 | 状态 | 说明 |
|------|:------:|:----:|------|
| **边界越界 — 机器人本体超出 polygon** | 🔴 高 | 🚧 待修复 | S2 top swath y=24.8085 → robot_half_width=0.375m 超出边界。修复尝试均导致 S7 crash（见下） |
| **S7 routing 拓扑极度脆弱** | 🔴 高 | 📋 已知 | 10 孔洞→仅 2 cell，任何改变 swath 数量/位置的改动均导致 "crosses obstacle holes"。不是代码 bug，是多边形+孔洞组合卡在临界状态 |
| **水平方向越界** | 🟡 中 | 📋 已知 | swath 端点 x=0.15/29.85 → robot body 超出 x=0/30。mid_hl=0.20 太小，改大需联动调整 coverage 参数 |
| **S7 16.7° 斜边 swath** | 🟡 中 | 📋 已知 | 来自 cell 主方向匹配了孔洞边缘角度，无法单独修复 |
...
| 方案 | 废弃日期 | 原因 |
|------|:--------:|------|
| **adaptive endpoint clamping** | 07-20 | S7 routing 太脆弱，端点缩进 → 穿洞。scaffold 保留在 `feat/visibility-graph-path-planning` 分支 |
| **孔洞边界补线 10° 角度过滤** | 07-20 | S7 斜边孔洞补线被 skip → 穿洞。外环 10° 检查保留 |
| **filterBoundarySwaths 预过滤** | 07-20 | 窄场景 S4 全删崩，覆盖丢失 |
