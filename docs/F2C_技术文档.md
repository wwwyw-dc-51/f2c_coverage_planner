...
> **编译状态**: 0 error 0 warning | **基准**: 7 场景零回归
...
| 参数 | 值 | 说明 |
|------|------|------|
| 车体宽度 | 0.95 m | robot_width |
| 覆盖宽度 | 0.45 m | coverage_width（刷盘直径） |
...
| **掉头规划** | Direct (Euclidean 直连) | Dubins、Reeds-Shepp | 差速驱动可原地转，Direct 最简单高效 |
...
| 维度 | Direct (当前) | Dubins |
|------|:---:|:---:|
| 连接方式 | Euclidean 直线 | 最小转弯半径曲线 |
| 适用场景 | 差速驱动（可原地转） | 阿克曼转向 |
| 路径点数 | 少 | 多（曲线离散化） |
| 计算速度 | 快 | 慢 |

差速驱动机器人最小转弯半径 ≈ 0，Direct 是最优选择。
...
Polygon (YAML/ROS2) → polygon_planner_node
              ...
              TSP genRoute → F2C::Route
                    │
              Direct 掉头 → F2C::Path (pre-RDP, ~2000 pts)
                    │
              ┌───────┴───────┐
              │               │
         RDP 简化          v7 段级检测
...
|------|---------|--------|------|:------:|------|
| 车体宽度 | `robot_width` | 0.95 | 0.3~2.0 m | **0.95** | 物理宽度，影响 headland 和转弯空间 |
| 覆盖宽度 | `coverage_width` | 0.45 | 0.2~1.0 m | **0.45** | 刷盘有效宽度，决定 swath 间距 |
| 最小转弯半径 | `min_turning_radius` | 0.01 | 0~1.0 m | **0.01** | 差速驱动≈0 |
...
| RDP 简化 | `path_simplify_enabled` | true | bool | **true** | 减少冗余路径点 |
...
### 5.2 最新基准 (v9, 2026-07-14)

| 场景 | 面积 | 覆盖率 | 得分 | gate | 说明 |
|------|:----:|:-----:|:----:|:----:|------|
| S1 矩形 20×15m | 300 | **99.91%** | **85.5** | 1.00 | 满分段 |
| S2 L 形 | 397 | **99.31%** | **84.3** | 1.00 | 满分段 |
| S3 含孔洞 | 458 | **99.09%** | **77.0** | 1.00 | 满分段 |
| S5 不规则 | 90 | **99.63%** | **82.0** | 1.00 | 满分段 |
| S6 多区域 | 84 | **98.33%** | **62.2** | 0.58 | 高分段 |
| notched 缺口孔 | 297 | **98.60%** | **69.9** | 0.72 | 高分段 |
| S4 窄走廊 | 40 | **90.03%** | **0.0** | 0.00 | 待修复 |

> 与 v8 稳定版完全一致，零回归。模块化重构未影响任何场景的覆盖质量。
...
| S5 | 不规则 | 90 m² | 0 | 复杂轮廓 |
| S6 | 多区域 | 84 m² | 2 | 多区域连接 |
| notched | 缺口 + 中心孔 | 297 m² | 1 | 缺口边界 + 孔洞 |
...
| 问题 | 严重度 | 状态 | 说明 |
|------|:------:|:----:|------|
| S4 窄走廊 ~90% | ⚠️ 中 | 待修复 | 固定宽度 swath 在 <2m 走廊无法整除；需动态重叠率 per-cell |
| 孔洞边界补刀 genRoute 丢弃 | ⚠️ 中 | 偶发 | fillBoundaryGaps 生成的孔洞补线可能被 TSP 路由丢弃 |
| `/tmp/` 硬编码路径 | 🔵 低 | 已知 | JSON 输出路径硬编码为 `/tmp/`，不兼容 Windows |
...
```
src/yingshi_robot/test_polygons/   — S1~S6 测试多边形
src/yingshi_robot/config/f2c_areas/ — notched 等实际场景
```
...
> **最后更新**: 2026-07-14 (v9 模块化重构 + fillBoundaryGaps 完整提取)
> **相关文档**: `SETUP_GUIDE.md` (发布包配置指南)、`模块化重构方案.md`、`综合进展报告_v8_2026-07-14.md`
