...
> 状态：三项确定性缺陷已在本地实现修复并补回归测试；尚未进行 VM 编译与场景回归；候选物理参数尚未写入运行脚本

## 1. 先读结论

本轮需要区分两类问题：

1. **与产品参数无关的确定性代码缺陷**：Route 边界补线插入顺序、route connection 被拉直、孔洞诊断环未闭合。本地代码均已修复，当前等待 VM 验证。
...
本地实现：提交 `59d6418` 新增 `appendConnectedSwath()`，在读取旧 route 尾点后一次性追加 connection 和独立 swath group。单元测试验证 group 数、connection 索引和补线归属；S6 场景结果仍待 VM 验证。
...
本地实现：提交 `15aa918` 新增 `planDirectPath()`，按 Route 折线逐段生成 TURN 状态，保留所有 connection 控制点；direct 模式不再调用 F2C `simplifyConnection()`，也跳过后续 RDP。回归测试包含两个连续直角拐点；notched/S5 场景仍待 VM 验证。
...
本地实现：提交 `548070e` 新增 `makeClosedRing()`，规划、路径修复、发布诊断和评估统一使用同一闭环构造；射线法同时改为显式处理末点到首点。矩形孔洞内外点测试已添加，日志结果仍待 VM 验证。
...
1. 在 VM 编译并运行 `planning_invariants_test`，确认三项本地修复通过。
