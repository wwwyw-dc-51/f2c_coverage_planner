...
private:
    ...
    // Calculate one generated swath length.
    ...
    f2c::types::Cells rectilinearDecompose(
        ...
        const f2c::types::Cell& grid_src) const
    {
        (void)grid_src;  // 不再使用原始边界，以生成边界为准
        RCLCPP_INFO(this->get_logger(),
                   "rectilinearDecompose: work_area has %zu rings (1 ext + %zu holes)",
                   work_area.size(), work_area.size() - 1);
        // 1. 收集网格坐标：仅从 work_area（生成边界）取顶点
        std::vector<double> xs, ys;
        auto collect = [&](const f2c::types::LinearRing& ring) {
            for (size_t i = 0; i + 1 < ring.size(); ++i) {
                xs.push_back(ring.getGeometry(i).getX());
                ys.push_back(ring.getGeometry(i).getY());
            }
        };
        collect(work_area.getExteriorRing());
        for (size_t ri = 0; ri + 1 < work_area.size(); ++ri)
            collect(work_area.getInteriorRing(ri));
        ...
        RCLCPP_DEBUG(this->get_logger(),
                    "Grid decompose: %zu x-lines × %zu y-lines (from work_area vertices)",
                    xs.size(), ys.size());

        // 2. 创建网格矩形，与 work_area 求交
        ...
        for (size_t i = 0; i + 1 < xs.size(); ++i) {
            for (size_t j = 0; j + 1 < ys.size(); ++j) {
                ...
                for (size_t ci = 0; ci < intersected.size(); ++ci) {
                    ...
                    // 逐 cell 处理孔洞：带内环的 cell 单独做减法
                    if (cell.size() > 1) {
                        RCLCPP_INFO(this->get_logger(),
                                   "  Cell %zu has %zu rings (1 ext + %zu holes), subtracting...",
                                   result.size(), cell.size(), cell.size() - 1);
                        ...
                        for (size_t hi = 0; hi + 1 < cell.size(); ++hi) {
                            ...
                            // buffer(0.001) 把裸内环扩展成有效多边形（有外环），否则 difference 无效果
                            hole_cells = hole_cells.buffer(0.001);
                            single = single.difference(hole_cells);
                        }
                        RCLCPP_INFO(this->get_logger(),
                                   "  After subtraction: %zu sub-cells", single.size());
                                     ...
                                     double hole_angle_tol_deg = 0.5) const
    {
        ...
        for (size_t ci = 0; ci < cells.size(); ++ci) {
            ...
            // 保留内部环（孔洞），用更保守的阈值避免变形
            for (size_t ri = 0; ri < c.size() - 1; ++ri) {
                clean.addRing(simplifyRing(c.getInteriorRing(ri), hole_angle_tol_deg));
            }
    ...
    double computeCellMainDirection(const f2c::types::Cell& cell) const
    {
        ...
        if (exterior_ring.size() < 2) {
            RCLCPP_WARN(this->get_logger(), "Cell has less than 2 points, using default angle 0");
            return 0.0;
        }
        ...
        RCLCPP_DEBUG(this->get_logger(), 
                    "Cell main direction: longest edge length=%.2f, angle=%.2f deg (%.2f rad)", 
                    max_length, main_angle * 180.0 / M_PI, main_angle);
    ...
    // 复杂度 O(N log N)，N=顶点数（去重排序），实际候选数通常 ≤ 10
        ...
        double dedup_tolerance_deg = 2.0) const
    {
        ...
        for (size_t i = 0; i < ring.size(); ++i) {
            ...
            if (len < 1e-9) continue;  // 忽略零长度边（退化多边形顶点）
        ...
        return deduped;  // 返回去重后的边缘方向弧度列表
    }

    // ========== 优化函数：提取分解角度候选（边缘垂直方向）==========
    ...
    {
        auto edge_angles = extractAllEdgeAngles(cell);
        ...
        double shrink_dist) const
    {
        ...
        if (!touches_outer) return;

        // ── swath 方向向量 ──
        double s_dx = std::cos(swath_angle);
        double s_dy = std::sin(swath_angle);
        // 边方向与 swath 方向夹角 < 5° 视为平行
        // 空隙永远在法向，只有平行于 swath 的边界边才能补法向空隙
        const double angle_tol = 0.996;  // cos(5°)
        ...
        RCLCPP_INFO(this->get_logger(),
            "  Boundary edge check: cell bbox=[%.1f,%.1f]-[%.1f,%.1f] angle=%.0f° n=%zu",
            c_min_x, c_min_y, c_max_x, c_max_y,
            swath_angle * 180.0 / M_PI, cell_swaths.size());
        ...
        for (size_t pi = 0; pi + 1 < poly_ring.size(); ++pi)
        {
            ...
            if (elen < 0.01) continue;  // 退化边

            // ── 边平行于 swath 方向？空隙在法向，只有平行边能补 ──
            ...
            for (size_t li = 0; li < inside.size(); ++li) {
                ...
                // ── 去重：检查是否与已有 swaths 重叠 ──
                // 在边法向上投影，距离 < cov_width*0.3 视为重叠 → 跳过
                double seg_cx = (seg.getGeometry(0).getX() + seg.getGeometry(1).getX()) * 0.5;
                double seg_cy = (seg.getGeometry(0).getY() + seg.getGeometry(1).getY()) * 0.5;
                double seg_proj = seg_cx * n_x + seg_cy * n_y;
                bool overlaps = false;
                for (size_t j = 0; j < cell_swaths.size(); ++j) {
                    double scx = (cell_swaths.at(j).startPoint().getX() +
                                  cell_swaths.at(j).endPoint().getX()) * 0.5;
                    double scy = (cell_swaths.at(j).startPoint().getY() +
                                  cell_swaths.at(j).endPoint().getY()) * 0.5;
                    double sproj = scx * n_x + scy * n_y;
                    if (std::abs(seg_proj - sproj) < cov_width * 0.3) {
                        overlaps = true;
                        break;
                    }
                }
                ...
                if (shrink_dist > 0)
                    fill_sw = adjustSwathEndpoints(fill_sw, shrink_dist);
                cell_swaths.push_back(fill_sw);
                RCLCPP_INFO(this->get_logger(),
                    "  Boundary edge swath: edge[%.1f,%.1f]→[%.1f,%.1f] +1 swath (len=%.2f m)",
                    px1, py1, px2, py2, seg_len);
            }
        }
    }
        ...
        try {
            ...
            double effective_no_hl_ratio = no_hl_width_ratio_;
            {
                ...
                for (size_t i = 0; i < n; ++i) {
                    ...
                    for (size_t j = i + 2; j < n; ++j) {
                        // 跳过相邻边和同一边
                        if (j == i || j == i_next || (j + 1) % n == i) continue;
                        // 计算边 i→i_next 与边 j→j_next 之间的最小距离
                        double di = std::hypot(
            ...
            if (decomposition_enabled_) {
                ...
                for (size_t ci = 0; ci < mid_hl.size(); ++ci) {
                    ...
                    auto sub = rectilinearDecompose(work_cell, grid_cell);
...
#if 0  // ═══════════════════════════════════════════════════════════════
                // ★ 后减法（已废弃）: 对每个 sub-cell 单独挖孔洞
                //   rectilinear 分解的交集操作已自然排除孔洞区域，
                //   sub-cell 永远不与孔洞重叠 → affected 永远为 0
                //   保留代码以供未来参考，编译跳过
                // ═══════════════════════════════════════════════════════════
                for (size_t ci = 0; ci < mid_hl.size(); ++ci) {
                    const auto& work_cell = mid_hl.getGeometry(ci);
                    if (work_cell.size() <= 1) continue;  // 无孔洞，跳过

                    RCLCPP_INFO(this->get_logger(),
                               "  Post-decomp hole cleanup: cell %zu has %zu hole(s), "
                               "checking %zu sub-cells...",
                               ci, work_cell.size() - 1, decomp_mid_hl.size());

                    // 为每个孔洞准备 buffered hole polygon
                    std::vector<f2c::types::Cells> buffered_holes;
                    for (size_t hi = 0; hi + 1 < work_cell.size(); ++hi) {
                        f2c::types::Cell hole_cell;
                        hole_cell.addRing(work_cell.getInteriorRing(hi));
                        f2c::types::Cells hc;
                        hc.addGeometry(hole_cell);
                        buffered_holes.push_back(hc.buffer(0.001));
                    }

                    f2c::types::Cells clean_decomp;
                    size_t affected = 0, removed = 0;
                    for (size_t si = 0; si < decomp_mid_hl.size(); ++si) {
                        f2c::types::Cells parts;
                        parts.addGeometry(decomp_mid_hl.getGeometry(si));
                        bool was_affected = false;
                        for (auto& bh : buffered_holes) {
                            auto before = parts.size();
                            parts = parts.difference(bh);
                            if (parts.size() != before) was_affected = true;
                        }
                        if (parts.size() == 0) {
                            ++removed;
                        } else {
                            if (was_affected) ++affected;
                            for (size_t pi = 0; pi < parts.size(); ++pi)
                                clean_decomp.addGeometry(parts.getGeometry(pi));
                        }
                    }
                    RCLCPP_INFO(this->get_logger(),
                               "  Hole cleanup: %zu affected, %zu removed, "
                               "%zu → %zu sub-cells",
                               affected, removed, decomp_mid_hl.size(), clean_decomp.size());
                    decomp_mid_hl = clean_decomp;
                }
#endif  // 后减法废弃代码
                ...
                no_hl = const_hl.generateHeadlands(mid_hl, no_hl_width_ratio_ * robot_width_);
            ...
            if (use_optimized_planner_ && no_hl.size() > 1) {
                const double ANGLE_MERGE_THRESHOLD = 45.0 * M_PI / 180.0;
                ...
                if (next_gid < static_cast<int>(nc)) {
                    ...
                    RCLCPP_INFO(this->get_logger(),
                               ...
                               ANGLE_MERGE_THRESHOLD * 180.0 / M_PI);
                    no_hl = merged;
                }
            }
            ...
            // ── 孔洞感知：cell 绕孔洞圆形排序 ──
            if (!last_holes_[index].empty() && swaths_by_cells.size() > 1) {
                double h_min_x = 1e9, h_min_y = 1e9, h_max_x = -1e9, h_max_y = -1e9;
                for (const auto& hole : last_holes_[index]) {
                    for (const auto& p : hole.points) {
                        if (p.x < h_min_x) h_min_x = p.x;
                        if (p.y < h_min_y) h_min_y = p.y;
                        if (p.x > h_max_x) h_max_x = p.x;
                        if (p.y > h_max_y) h_max_y = p.y;
                    }
                }
                double h_cx = (h_min_x + h_max_x) * 0.5;
                double h_cy = (h_min_y + h_max_y) * 0.5;

                // 计算每个 cell 的质心（从 swaths 平均）
                std::vector<std::pair<double, size_t>> cell_angles;
                for (size_t ci = 0; ci < swaths_by_cells.size(); ++ci) {
                    double sx = 0, sy = 0;
                    size_t n = 0;
                    for (size_t j = 0; j < swaths_by_cells.at(ci).size(); ++j) {
                        const auto& sw = swaths_by_cells.at(ci).at(j);
                        sx += (sw.startPoint().getX() + sw.endPoint().getX()) * 0.5;
                        sy += (sw.startPoint().getY() + sw.endPoint().getY()) * 0.5;
                        ++n;
                    }
                    if (n > 0) {
                        cell_angles.push_back({std::atan2(sy/n - h_cy, sx/n - h_cx), ci});
                    }
                }
                std::sort(cell_angles.begin(), cell_angles.end());

                // 按圆形顺序重建 swaths_by_cells
                f2c::types::SwathsByCells ordered;
                std::ostringstream order_str;
                for (const auto& [ang, ci] : cell_angles) {
                    ordered.push_back(swaths_by_cells.at(ci));
                    if (order_str.tellp() > 0) order_str << " → ";
                    order_str << "c" << ci << "@" << std::fixed << std::setprecision(0)
                              << (ang * 180.0 / M_PI) << "°";
                }
                swaths_by_cells = ordered;
                RCLCPP_INFO(this->get_logger(),
                    "Cell traversal order (circular around hole): %s", order_str.str().c_str());
            }
            ...
            // 第五步：用 genRoute 生成路由（含 headland 路径，避免越界）
            ...
            f2c::types::Route route = route_planner.genRoute(
            ...
            // ── 路径-孔洞交叉诊断（点级 + 线段级）──
            {
                ...
                if (!hole_check_rings.empty()) {
                    ...
                    auto pointInHole = [&](double px, double py) -> bool {
                        for (const auto& hr : hole_check_rings) {
                            bool inside = false;
                            size_t n = hr.size();
                            for (size_t vi = 0, vj = n - 1; vi < n; vj = vi++) {
                                double xi = hr.getGeometry(vi).getX();
                                double yi = hr.getGeometry(vi).getY();
                                double xj = hr.getGeometry(vj).getX();
                                double yj = hr.getGeometry(vj).getY();
                                if (((yi > py) != (yj > py)) &&
                    ...
                    auto checkSegment = [&](double x0, double y0, double x1, double y1) -> bool {
                        for (int k = 1; k <= SEG_SAMPLES; ++k) {
                            double t = static_cast<double>(k) / (SEG_SAMPLES + 1);
                            if (pointInHole(x0 + t*(x1-x0), y0 + t*(y1-y0))) return true;
                        }
                        return false;
                    };
            ...
            if (eval_enable_report_) {
                ...
                EvalResult eval_result = evaluatePlan(
                    path_pre_rdp, swaths, no_hl, hole_rings,
