...
// v9.12 Cell 排序：保留 F2C 原生排序，只在四种合法入口变体中优化
// 连接代价；有孔洞时对固定极角顺序做全链动态规划，无孔洞时做
// Cell/入口联合贪心。
    ...
    const std::string& swath_order_type)
{
    ...
    // 后续修复要求：孔洞裁剪后可能留下空 Cell。把非空 Cell 压缩后
    // 运行 v9.12 排序，再把空槽位追加回末尾，避免 DP/入口选择解引用空组。
    std::vector<size_t> active_original_indices;
    f2c::types::SwathsByCells active_cells;
    for (size_t ci = 0; ci < n_cells; ++ci) {
        if (swaths_by_cells[ci].size() == 0) continue;
        active_original_indices.push_back(ci);
        active_cells.push_back(swaths_by_cells[ci]);
    }
    if (active_cells.size() != n_cells) {
        if (active_cells.size() == 0) return;

        std::vector<size_t> active_order;
        greedyCellOrder(
            active_cells, active_order, hole_rings, swath_order_type);

        std::vector<size_t> mapped_order;
        mapped_order.reserve(n_cells);
        for (const size_t active_index : active_order) {
            mapped_order.push_back(active_original_indices.at(active_index));
        }
        for (size_t ci = 0; ci < n_cells; ++ci) {
            if (swaths_by_cells[ci].size() == 0) {
                mapped_order.push_back(ci);
            }
        }

        swaths_by_cells = active_cells;
        for (size_t ci = 0; ci < n_cells; ++ci) {
            if (swaths_by_cells.size() == n_cells) break;
            swaths_by_cells.push_back(f2c::types::Swaths());
        }
        cell_order = mapped_order;
        return;
    }
    ...
    if (!hole_rings.empty()) {
        ...
        for (size_t ci = 0; ci < n_cells; ++ci) {
            ...
            for (size_t j = 0; j < cell.size(); ++j) {
                sx += (cell.at(j).startPoint().getX() +
                       cell.at(j).endPoint().getX()) * 0.5;
                sy += (cell.at(j).startPoint().getY() +
                       cell.at(j).endPoint().getY()) * 0.5;
                ++sn;
            }
            double ang = (sn > 0)
                ? std::atan2(sy / sn - h_cy, sx / sn - h_cx)
                : 0.0;
        ...
        for (size_t i = 0; i < n_cells; ++i) {
            traversal_order.push_back(
                cell_angles[(c0_pos + i) % n_cells].second);
        }
    } else {
    ...
    while (new_order.size() < n_cells) {
        ...
        if (choice.cost >= std::numeric_limits<double>::max() * 0.5) break;
        ...
        if (selected_cell.size() > 0) {
            const auto& last_swath = selected_cell[selected_cell.size() - 1];
