            ...
            // 此处直接向 route 追加边界填充 swath，保证出发侧和到达侧边缘覆盖。
            {
                ...
                if (outer_ring.size() >= 4 && swaths_by_cells.size() > 0) {
                    ...
                    if (route.sizeVectorSwaths() > 0 && route.getSwaths(0).size() > 0) {
                        auto& fs = route.getSwaths(0).at(0);
                        double fdx = fs.endPoint().getX() - fs.startPoint().getX();
                        double fdy = fs.endPoint().getY() - fs.startPoint().getY();
                        double flen = std::hypot(fdx, fdy);
                        if (flen > 0.01) { s_dx = fdx / flen; s_dy = fdy / flen; }
                    }
                    double proj_min = 1e9, proj_max = -1e9;

                    // 计算已有 route swaths 的投影范围
                    for (size_t ri = 0; ri < route.sizeVectorSwaths(); ++ri) {
                        auto& rsw = route.getSwaths(ri);
                        for (size_t rj = 0; rj < rsw.size(); ++rj) {
                            double cx = (rsw.at(rj).startPoint().getX()
                                       + rsw.at(rj).endPoint().getX()) * 0.5;
                            double cy = (rsw.at(rj).startPoint().getY()
                                       + rsw.at(rj).endPoint().getY()) * 0.5;
                    ...
                    // 遍历外环边界，补上缺失的边界线
                    for (size_t pi = 0; pi + 1 < outer_ring.size(); ++pi) {
                        double px1 = outer_ring.getGeometry(pi).getX();
                        double py1 = outer_ring.getGeometry(pi).getY();
                        double px2 = outer_ring.getGeometry(pi + 1).getX();
                        double py2 = outer_ring.getGeometry(pi + 1).getY();
                        double edx = px2 - px1, edy = py2 - py1;
                        double elen = std::hypot(edx, edy);
                        if (elen < 0.01) continue;
                        // 仅处理与 swath 方向平行的边
                        double cos_a = std::abs(edx * s_dx + edy * s_dy) / elen;
                        if (cos_a < 0.9397) continue;  // cos(20°)

                        // 边中点，内侧偏移
                        double n_x = -edy / elen, n_y = edx / elen;
                        double emx = (px1 + px2) * 0.5, emy = (py1 + py2) * 0.5;
                        double ccx = 0, ccy = 0;
                        for (size_t ci = 0; ci + 1 < outer_ring.size(); ++ci) {
                            ccx += outer_ring.getGeometry(ci).getX();
                            ccy += outer_ring.getGeometry(ci).getY();
                        }
                        ccx /= (outer_ring.size() - 1);
                        ccy /= (outer_ring.size() - 1);
                        ...
                        // 投影在已有范围之外 → 缺失的边界线
                        // 边界填充应位于 swath 范围最外侧，只要 bproj 明显超出 proj_max
                        // 或明显低于 proj_min，就是缺失的边界补刀
                        bool beyond_max = (proj_max - proj_min > coverage_width_) &&
                                          (bproj > proj_max + coverage_width_ * 0.05);
                        bool below_min = (proj_max - proj_min > coverage_width_) &&
                                         (bproj < proj_min - coverage_width_ * 0.05);
                        if (beyond_max || below_min) {
                            // 生成边界填充 swath 并追加到 route
